...
const makeHashComparer = (): HashComparer => {
    class HashComparerStub implements HashComparerStub {
...
describe('DbAuthentication UseCase', () => {
    ...
    test('Should call return a token on succes', async () => {
        const { sut } = makeSut()
        const accessToken = await sut.auth(makeFakeAuthentication())
    ...
    test('Should call UpdateAccessTokenRepository with correct values', async () => {
        ...
        const updateSpy = jest.spyOn(updateAcessTokenRepositoryStub, 'updateAccesToken')
        await sut.auth(makeFakeAuthentication())
    ...
    test('Should throw if UpdateAccessTokenRepository throws', async () => {
        const { sut, updateAcessTokenRepositoryStub } = makeSut()
        jest.spyOn(updateAcessTokenRepositoryStub, 'updateAccesToken').mockReturnValueOnce(new Promise((resolve, reject) => reject(new Error)))
        const promise = sut.auth(makeFakeAuthentication())
        await expect(promise).rejects.toThrow()
    })
